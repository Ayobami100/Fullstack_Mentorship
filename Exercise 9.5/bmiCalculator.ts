export const calculateBmi = (ht: number, weight: number): any => {
  const bmi = weight / (ht / 100) ** 2;
...
if (isCalledDirectly) {
  try {
    const { ht, weight } = parseArguments(process.argv);
    console.log(calculateBmi(ht, weight));
  } catch (e) {
    console.log("An error has occured: ", e.message);
    console.log("USAGE: npm run calculateBmi ht(cm) weight(kg)");
  }
}